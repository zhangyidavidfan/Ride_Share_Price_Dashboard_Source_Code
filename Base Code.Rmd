...
prelim_model = lm(Fare~TripsPooled + Start_Time + Start_AM_PM + AvgTemp + MaxWindSpeed + MaxHumidity + AvgPrecipation + is_holiday + Population + Median_Household_Income + Pop_Below_Poverty + Owner_Occupied + Renter_Occupied_House + distance + Weekdays, data = train)
```

```{r}
#Train LM Model on Time
prelim_model_time = lm(TripSeconds~TripsPooled + Start_Time + Start_AM_PM + AvgTemp + MaxWindSpeed + MaxHumidity + AvgPrecipation + is_holiday + Population + Median_Household_Income + Pop_Below_Poverty + Owner_Occupied + Renter_Occupied_House + distance + Weekdays, data = train)
```
